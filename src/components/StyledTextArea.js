...
const InnerTextarea = React.forwardRef(function InnerTextarea({ label, ...props }, ref) {
  ...
  export default function FloatingLabelTextarea({ label }) {
    return (
      <Textarea
        slots={{ textarea: InnerTextarea }}
        slotProps={{ textarea: { placeholder: 'A placeholder', label: label } }}
        sx={{ borderRadius: '6px' }}
      />
    );
  }